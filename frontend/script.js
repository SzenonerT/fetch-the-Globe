...
async function fetchAllCountryName(url) {
  const response = await fetch(url);
  const countries = await response.json();
  return countries;
}
const fetchCountryByCode = async (event) => {
  const code = event.currentTarget.textContent.slice(
    event.currentTarget.textContent.indexOf("[") + 1,
    event.currentTarget.textContent.indexOf("]")
  );
  console.log(code);
  const response = await fetch(
    `https://jvvkjy8utk.execute-api.eu-central-1.amazonaws.com/tourist/api/countries/by-cca3/%7Bcca3%7D$(code)`
  );
